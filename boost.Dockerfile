...
RUN . /profile && wget https://boostorg.jfrog.io/artifactory/main/release/${BOOST_VERSION}/source/boost_${BOOST_VERSION_MOD}.tar.bz2
RUN . /profile && tar --bzip2 -xf boost_${BOOST_VERSION_MOD}.tar.bz2

RUN . /profile && cd boost_${BOOST_VERSION_MOD} && \
